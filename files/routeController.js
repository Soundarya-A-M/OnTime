...
// Get all routes
// FIX: default filter to isActive: true so soft-deleted routes don't appear everywhere
export const getAllRoutes = async (req, res) => {
    try {
        const { isActive } = req.query;

        // Default to only active routes unless caller explicitly passes isActive=false
        const filter = {
...
export const updateRoute = async (req, res) => {
    try {
        const {
            routeName, routeNumber, stops, distance, estimatedDuration, isActive,
            sourceCity, destinationCity, sourceCoordinates, destinationCoordinates, polyline
        } = req.body;
...
// Delete route (Admin only) — hard delete, not soft-delete
// FIX: was soft-delete (isActive: false) but getAllRoutes now filters isActive: true by default,
//      making soft-deleted routes invisible. Use hard delete for true data hygiene.
